...
export const conceptDiagrams: Record<string, DiagramSpec> = {
  ...
  'interactive-chart-vs-app': {
    ...
    relax: false,
    groups: [
      { id: 'chart', label: 'Interactive chart', x: 0.05, y: 0.14, w: 0.43, h: 0.80, stroke: vizTheme.primary },
      { id: 'app', label: 'Data app', x: 0.52, y: 0.14, w: 0.43, h: 0.80, stroke: vizTheme.cyan },
    ],
    nodes: [
      { id: 'A', label: 'Interactive chart\n(single artifact)', x: 0.26, y: 0.34, w: 260, h: 74, variant: 'primary' },
      { id: 'B', label: 'Hover · zoom\nlegend filter', x: 0.26, y: 0.61, w: 250, h: 70, variant: 'note' },
      { id: 'C', label: 'Export HTML', x: 0.26, y: 0.85, w: 220, h: 62, variant: 'accent' },

      { id: 'D', label: 'Data app\n(many views)', x: 0.74, y: 0.30, w: 260, h: 70, variant: 'primary' },
      { id: 'E', label: 'Controls\n(inputs)', x: 0.62, y: 0.60, w: 200, h: 66, variant: 'note' },
      { id: 'F', label: 'Callback\n(compute)', x: 0.86, y: 0.60, w: 200, h: 66, variant: 'note' },
      { id: 'G', label: 'Outputs\n(charts + tables)', x: 0.74, y: 0.85, w: 300, h: 74, variant: 'ok' },
    ],
    edges: [
      { from: 'A', to: 'B', stroke: 'rgba(147,197,253,0.82)' },
      { from: 'B', to: 'C', stroke: 'rgba(147,197,253,0.82)' },

      { from: 'D', to: 'E', dashed: true, stroke: 'rgba(148,163,184,0.58)' },
      { from: 'E', to: 'F', stroke: 'rgba(147,197,253,0.82)' },
      { from: 'F', to: 'G', stroke: 'rgba(147,197,253,0.82)' },
      { from: 'G', to: 'E', dashed: true, stroke: 'rgba(34,211,238,0.82)' },
    ],
  },
  ...
  'linked-views-reading-order': {
    ...
    relax: false,
    nodes: [
      { id: 'A', label: 'Overview\nsee trends', x: 0.12, y: 0.56, w: 220, h: 72, variant: 'primary' },
      { id: 'B', label: 'Brush\nchoose a range', x: 0.38, y: 0.56, w: 220, h: 72, variant: 'note' },
      { id: 'C', label: 'Details\ncompare distributions', x: 0.64, y: 0.56, w: 220, h: 72, variant: 'primary' },
      { id: 'D', label: 'Lookup\ninspect records', x: 0.90, y: 0.56, w: 220, h: 72, variant: 'accent' },
    ],
    edges: [
      { from: 'A', to: 'B' },
      { from: 'B', to: 'C' },
  ...
  'plotly-mental-model': {
    ...
    relax: false,
    nodes: [
      { id: 'A', label: 'Python code\n(Plotly Express / graph_objects)', x: 0.50, y: 0.18, w: 380, h: 70, variant: 'note' },
      { id: 'B', label: 'Figure spec\n(JSON-like)', x: 0.50, y: 0.41, w: 300, h: 70, variant: 'primary' },
      { id: 'C', label: 'HTML artifact', x: 0.50, y: 0.64, w: 250, h: 64, variant: 'accent' },
      { id: 'D', label: 'plotly.js renders\nSVG + interactions', x: 0.50, y: 0.87, w: 360, h: 70, variant: 'ok' },
    ],
    edges: [
      { from: 'A', to: 'B' },
      { from: 'B', to: 'C' },
  ...
  'chart-to-app-loop': {
    ...
    relax: false,
    nodes: [
      { id: 'A', label: 'Data\n(table)', x: 0.16, y: 0.72, w: 200, h: 66, variant: 'primary' },
      { id: 'B', label: 'Compute\n(transform + aggregate)', x: 0.44, y: 0.72, w: 270, h: 74, variant: 'note' },
      { id: 'C', label: 'State\n(filters, selections)', x: 0.44, y: 0.32, w: 280, h: 74, variant: 'primary' },
      { id: 'D', label: 'Views\n(charts + tables)', x: 0.82, y: 0.72, w: 280, h: 74, variant: 'ok' },
    ],
    edges: [
      { from: 'A', to: 'B' },
      { from: 'C', to: 'B', stroke: 'rgba(147,197,253,0.78)' },
      { from: 'B', to: 'D' },
      { from: 'D', to: 'C', dashed: true, stroke: 'rgba(34,211,238,0.82)' },
    ],
  },
  ...
  'dash-layout-tree': {
    ...
    relax: false,
    nodes: [
      { id: 'A', label: 'app.layout', x: 0.50, y: 0.12, w: 230, h: 62, shape: 'pill', variant: 'primary' },
      { id: 'B', label: 'Div (page)', x: 0.50, y: 0.34, w: 240, h: 62, shape: 'pill', variant: 'note' },
      { id: 'C', label: 'Controls', x: 0.30, y: 0.58, w: 200, h: 62, variant: 'primary' },
      { id: 'D', label: 'Outputs', x: 0.70, y: 0.58, w: 200, h: 62, variant: 'primary' },
      { id: 'E', label: 'Dropdown', x: 0.16, y: 0.82, w: 180, h: 60, variant: 'muted' },
      { id: 'F', label: 'RangeSlider', x: 0.38, y: 0.82, w: 180, h: 60, variant: 'muted' },
      { id: 'G', label: 'Graph', x: 0.62, y: 0.82, w: 180, h: 60, variant: 'muted' },
      { id: 'H', label: 'Table', x: 0.84, y: 0.82, w: 180, h: 60, variant: 'muted' },
    ],
    edges: [
